...
const RefreshIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.95"/></svg>;
...
const ChevronIcon = ({ open }) => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}><polyline points="6 9 12 15 18 9"/></svg>;
...
function QuestionsTab({ questions, loading, error, refresh, navigate, stats }) {
  ...
  function expandAll()   { const o = {}; sortedTopics.forEach(t => o[t] = true);  setExpandedTopics(o); }
  function collapseAll() { setExpandedTopics({}); }
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.4rem", marginBottom: "1rem" }}>
        {[
          ...
          <div key={s.label} className="card" style={{ textAlign: "center", padding: "0.5rem 0.25rem" }}>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem" }}>
        <button className="btn btn-primary" style={{ gap: "0.4rem", padding: "0.75rem 1rem", flexShrink: 0 }} onClick={() => navigate("/admin/add")}>
          <PlusIcon /> Nova
        </button>
        <input
          value={search}
      ...
      {sortedTopics.length > 1 && (
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem" }}>
          <button onClick={anyExpanded ? collapseAll : expandAll} style={{
      ...
      {loading && (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {[1,2,3].map(i => (
            <div key={i} style={{ height: 48, borderRadius: "var(--radius)", background: "var(--border)", opacity: 0.3 }} />
          ))}
        </div>
      )}
      ...
      {!loading && sortedTopics.length === 0 && (
        <div style={{ textAlign: "center", padding: "2.5rem 0", color: "var(--text3)" }}>
          {search ? "Nenhuma questão encontrada." : "Nenhuma questão cadastrada ainda."}
        </div>
      )}
            ...
            return (
              <div key={topic} style={{
                ...
                >
                  ...
                  <span style={{ flex: 1, textAlign: "left", fontWeight: 700, fontSize: "0.9rem", color: isOpen ? color : "var(--text)" }}>
                    {topic}
                  </span>
                  <span style={{
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                    {qs.map((q, qi) => (
                      <div key={q.id} style={{
                        ...
                        background: qi % 2 === 0 ? "var(--bg3)" : "var(--card)",
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <span style={{
                            ...
                            color: TYPE_COLOR[q.type], background: TYPE_COLOR[q.type] + "20",
...
function UsersTab() {
  ...
  async function fetchPatterns() {
    const { data } = await supabase.from("banned_names").select("*").order("created_at", { ascending: false });
    setBannedNames(data ?? []);
  }
  ...
  async function handleBan() {
    ...
    const { error } = await supabase
      .from("users")
      .update({ banned: true, ban_reason: banReason.trim() || "Violação das regras" })
      .eq("username", banModal.username);
    if (error) { showToast("Erro ao banir.", "error"); return; }
  ...
  async function handleUnban(username) {
    ...
    if (error) { showToast("Erro ao desbanir.", "error"); return; }
  ...
  async function handleAddPattern() {
    ...
    const { error } = await supabase
      .from("banned_names")
      .insert([{ pattern: newPattern.trim().toLowerCase(), reason: "Bloqueado pelo admin" }]);
    if (error) {
      showToast(error.message.includes("unique") ? "Padrão já existe." : "Erro ao adicionar.", "error");
      return;
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", gap: "0.5rem" }}>
        ...
        <TabBtn active={tab2 === "patterns"} onClick={() => setTab2("patterns")}>
      ...
      {tab2 === "users" && (
        <>
          ...
          {!loading && filteredUsers.length === 0 && (
            <p style={{ textAlign: "center", color: "var(--text3)", padding: "1.5rem 0", fontSize: "0.9rem" }}>
            ...
            {filteredUsers.map(u => (
              <div key={u.id} style={{
                ...
                <div style={{
                  ...
                  background: u.banned ? "rgba(239,68,68,0.15)" : "var(--bg3)",
                  border: `2px solid ${u.banned ? "var(--red)" : "var(--border)"}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: "0.8rem", color: u.banned ? "var(--red)" : "var(--text3)",
                }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                    <span style={{ fontSize: "0.9rem", fontWeight: 600, color: u.banned ? "var(--red)" : "var(--text)" }}>
                      {u.username}
                    </span>
                    {u.banned && (
                      <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "var(--red)", background: "rgba(239,68,68,0.15)", padding: "0.1rem 0.4rem", borderRadius: "999px" }}>
                        BANIDO
                      </span>
                    )}
                  </div>
                  {u.ban_reason && (
                    <div style={{ fontSize: "0.75rem", color: "var(--text3)" }}>Motivo: {u.ban_reason}</div>
                  )}
      ...
      {tab2 === "patterns" && (
        <>
          <p style={{ fontSize: "0.85rem", color: "var(--text3)", margin: 0 }}>
            Qualquer nome que <strong>contenha</strong> um desses padrões será bloqueado no cadastro.
          </p>
          ...
          {bannedNames.length === 0 && (
            <p style={{ textAlign: "center", color: "var(--text3)", fontSize: "0.85rem", padding: "1rem 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            {bannedNames.map(b => (
              <div key={b.id} style={{
                ...
                <span style={{ fontSize: "0.85rem", color: "#fca5a5", fontFamily: "monospace", flex: 1 }}>
                  🚫 {b.pattern}
                </span>
                {b.reason && (
                  <span style={{ fontSize: "0.72rem", color: "var(--text3)" }}>{b.reason}</span>
                )}
                <button className="icon-btn danger" onClick={() => handleRemovePattern(b.id)} title="Remover">
                  <TrashIcon />
                </button>
              </div>
            ))}
      ...
      {banModal && (
        <div className="modal-overlay" onClick={() => setBanModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3 style={{ marginBottom: "0.5rem" }}>Banir "{banModal.username}"?</h3>
            ...
            <div style={{ display: "flex", gap: "0.75rem" }}>
              <button className="btn btn-ghost btn-full" onClick={() => setBanModal(null)}>Cancelar</button>
              <button className="btn btn-danger btn-full" onClick={handleBan}>Banir</button>
            </div>
          </div>
        </div>
      )}
